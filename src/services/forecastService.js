...
export function getCurrentWeather(location) {
  return new Promise((resolve, reject) => {
    fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${location.lat}&longitude=${location.long}&current=temperature_2m,relative_humidity_2m,apparent_temperature,precipitation,weather_code,cloud_cover,pressure_msl,wind_speed_10m&wind_speed_unit=ms`,
    )
      .then((response) => {
        if (response.ok) {
          return response.json()
...
function transformCurrent(weather) {
  ...
  curWeather.temp = {
    temp: weather.current.temprature_2m,
    temp_unit: weather.current_units.temprature_2m,
    humidity: weather.current.relative_humidity_2m,
    humidity_unit: weather.current_units.humidity_2m,
